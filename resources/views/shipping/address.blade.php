    ...
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
        <div>
            <label for="shipping_data_address_country_id" class="block text-sm font-medium text-gray-700">
                {{ __('order::common.country') }}
            </label>
            <select id="shipping_data_address_country_id" name="shipping_data[address][country_id]"
                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                <option value="">{{ __('order::common.select_country') }}</option>
                @foreach($countries as $country)
                    <option value="{{ $country->id }}"
                            @if(old('shipping_data.address.country_id', $address['country_id'] ?? $defaultCountryId) == $country->id) selected @endif>
                        {{ $country->name }}
                    </option>
                @endforeach
            </select>
